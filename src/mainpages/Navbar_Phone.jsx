  ...
  const toggleMenu = () => {
    if (!isMenuOpen) {
      ...
      tl.to(line1Ref.current, { width: "32px", duration: 0.2 })
        ...
        .to(
          line2Ref.current,
          { scaleX: 1, rotation: -45, duration: 0.3, ease: "back.out(2)" },
          "<"
        );

      // Ensure display is block before animating opacity
      gsap.fromTo(
        menuRef.current,
        { display: "block", x: 60, opacity: 0 },
      ...
      // CLOSE
      const tl = gsap.timeline();
      tl.to(line2Ref.current, { scaleX: 0, rotation: 0, duration: 0.3 }).to(
        line1Ref.current,
        { rotation: 0, width: "32px", duration: 0.3 },
        "<"
      );
      ...
      gsap.to(menuRef.current, {
        ...
        onComplete: () => {
          setIsMenuOpen(false);
          // SECURITY FIX: Force display none after animation finishes
    ...
    >
      ...
      <div className="fixed top-4 right-4 z-50 pointer-events-auto">
        ...
        >
          ...
          />
          <div
            className={`absolute inset-0 rounded-full hover:bg-white/10 transition-colors duration-300`}
          />
      ...
      {/* --- DROPDOWN MENU --- */}
      {/* SECURITY FIX: Added logic to switch pointer-events based on isMenuOpen */}
      <div
        ref={menuRef}
        className={`fixed top-24 right-4 w-[280px] py-10 px-8 rounded-3xl shadow-2xl hidden origin-top-right z-40 ${
          currentTheme.menuBg
        } 
